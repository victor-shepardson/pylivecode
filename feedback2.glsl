...
vec4 blur6pt(vec2 px){
  const vec3 d = vec3(1.,-1.,0.);
  return (
    4.*samp(history_0, px)
    + 2.*samp(history_1, px)
    + samp(history_0, px+d.xz)
    + samp(history_0, px+d.yz)
    + samp(history_0, px+d.zx)
    + samp(history_0, px+d.zy)
    )/10.;
}
...
void main() {
  ...
  vec4 c_sv = 0.01*sin(2.*pi*(c_acc.abgr+p.xxxy*vec4(1.,2.,3.,1.)));

  // vec4 c0 = blur6pt(px);
  vec4 c0 = mix(c_in, blur6pt(px), 0.8);
  vec4 c1 = fract(c_sv + 1. + .1*c0 + c_acc).argb;
  // for(int i=0;i<4;i++){
  //   if (c1[i] > 1.)
  //     c1[i] = 1.-c0.gbar[i];
  //   else if(c1[i] < 0.)
