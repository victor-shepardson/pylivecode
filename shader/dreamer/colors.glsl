...
void main()
{
    ...
    vec4 r = texture2D(displacements, wrap(uv));
    vec2 r1 = r.xy;
    vec2 r2 = r.zw;
    vec4 c0 = texture2D(history_0, wrap(uv));
    vec4 c1 = texture2D(history_0, wrap(uv+r1*d.xy));
    vec4 c2 = texture2D(history_0, wrap(uv+r2*d.xy));
    ...
    gl_FragColor = c0 - alpha_c*dJdc;

    //if(iFrame==0 || iMouse.z>0.){ //initial condition
    if(frame==0){
        gl_FragColor = cos(pi*rad+vec4(0., 1./4., 1./2., 3./4.));
        //gl_FragColor = vec4(cos(pi*rad+vec3(0., 2./3., 4./3.)),0.);
        //gl_FragColor = vec4(cos(pi*2.*(uv.xxy+vec3(0.,0.5,0.))).gbr, 0.);
    	//gl_FragColor = vec4(sin(pi*2.*vec3(1.,6.,7.)*(uv.yxx+vec3(0.,0.,0.25))).gbr, 0.);
        //gl_FragColor = texture2D(iChannel2, uv)*2.-1.;
    }
}
