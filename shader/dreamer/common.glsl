...
const float lambda_b = 0.0625;//8e-1;
...
vec2 pol2car(vec2 x){
 	return x.x*cos(x.y-vec2(0.,pi/2.));
}
...
vec3 hsl_mix(vec3 x, vec3 y, float m){
    if(x.r-y.r > 180.) y.r+=360.;
    else if(y.r-x.r > 180.) x.r+=360.;
    vec3 c = mix(x, y, m);
    if (c.r>360.) c.r-=360.;
    return c;
}

vec3 hsl_mix2(vec3 x, vec3 y, float m){
    vec3 c = mix(x, y, m);
    if (c.r>=360.) c.r -= 360.*floor(c.r/360.);
    if (c.r<0.) c.r -= 360.*(floor(c.r/360.)-1.);
    return c;
}
